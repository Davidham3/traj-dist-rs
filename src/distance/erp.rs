...
//! - `erp_standard_with_dist_type`: Correct implementation with cumulative initialization
//! - `erp_compat_with_dist_type`: Compatible implementation that matches the bug in the original traj-dist library
//! 
//! ## Complexity
//! 
...
/// 
/// ```rust
/// use traj_dist_rs::{erp_standard_with_dist_type, DistanceType};
/// 
/// let traj1 = vec![[0.0, 0.0], [1.0, 1.0]];
/// let traj2 = vec![[0.0, 1.0], [1.0, 0.0]];
/// let reference_point = [0.0, 0.0];
/// 
/// let distance = erp_standard_with_dist_type(&traj1, &traj2, reference_point, DistanceType::Euclidean);
/// println!("ERP distance (standard): {}", distance);
/// ```
pub fn erp_standard_with_dist_type<T: CoordSequence<Coord = [f64; 2]>>(
    t0: &T,
    t1: &T,
    g: [f64; 2],
...
/// For correct ERP calculations, use `erp_standard_with_dist_type` instead.
/// 
/// # Arguments
/// 
/// * `t0` - The first trajectory to compare
...
/// 
/// ```rust
/// use traj_dist_rs::{erp_compat_with_dist_type, DistanceType};
/// 
/// let traj1 = vec![[0.0, 0.0], [1.0, 1.0]];
/// let traj2 = vec![[0.0, 1.0], [1.0, 0.0]];
/// let reference_point = [0.0, 0.0];
/// 
/// let distance = erp_compat_with_dist_type(&traj1, &traj2, reference_point, DistanceType::Euclidean);
/// println!("ERP distance (compat): {}", distance);
/// ```
pub fn erp_compat_with_dist_type<T: CoordSequence<Coord = [f64; 2]>>(
    t0: &T,
    t1: &T,
    g: [f64; 2],
    dist_type: DistanceType,
...
/// Computes ERP distance using the standard implementation with Euclidean distance
/// 
/// # Deprecated
/// 
/// This function is deprecated. Use `erp_standard_with_dist_type()` with `DistanceType::Euclidean` instead.
#[deprecated(note = "Use erp_standard_with_dist_type() with DistanceType instead")]
pub fn erp_standard<T: CoordSequence<Coord = [f64; 2]>>(t0: &T, t1: &T, g: [f64; 2]) -> f64 {
    erp_standard_with_dist_type(t0, t1, g, DistanceType::Euclidean)
}

/// Computes ERP distance using the traj-dist compatible implementation with Euclidean distance
/// 
/// # Deprecated
/// 
/// This function is deprecated. Use `erp_compat_with_dist_type()` with `DistanceType::Euclidean` instead.
#[deprecated(note = "Use erp_compat_with_dist_type() with DistanceType instead")]
pub fn erp_compat_traj_dist<T: CoordSequence<Coord = [f64; 2]>>(
    t0: &T,
    t1: &T,
    g: [f64; 2],
) -> f64 {
    erp_compat_with_dist_type(t0, t1, g, DistanceType::Euclidean)
}

/// Computes ERP distance using the standard implementation with Spherical distance
/// 
/// # Deprecated
/// 
/// This function is deprecated. Use `erp_standard_with_dist_type()` with `DistanceType::Spherical` instead.
#[deprecated(note = "Use erp_standard_with_dist_type() with DistanceType instead")]
pub fn erp_standard_spherical<T: CoordSequence<Coord = [f64; 2]>>(
    t0: &T,
    t1: &T,
    g: [f64; 2],
) -> f64 {
    erp_standard_with_dist_type(t0, t1, g, DistanceType::Spherical)
}

/// Computes ERP distance using the traj-dist compatible implementation with Spherical distance
/// 
/// # Deprecated
/// 
/// This function is deprecated. Use `erp_compat_with_dist_type()` with `DistanceType::Spherical` instead.
#[deprecated(note = "Use erp_compat_with_dist_type() with DistanceType instead")]
pub fn erp_compat_traj_dist_spherical<T: CoordSequence<Coord = [f64; 2]>>(
    t0: &T,
    t1: &T,
    g: [f64; 2],
) -> f64 {
    erp_compat_with_dist_type(t0, t1, g, DistanceType::Spherical)
}
...
mod tests {
    ...
    fn test_erp_euclidean_simple() {
        ...
        let dist_compat = erp_compat_with_dist_type(&t0, &t1, g, DistanceType::Euclidean);
        let dist_standard = erp_standard_with_dist_type(&t0, &t1, g, DistanceType::Euclidean);
    ...
    fn test_erp_spherical_simple() {
        ...
        let dist_compat = erp_compat_with_dist_type(&t0, &t1, g, DistanceType::Spherical);
        let dist_standard = erp_standard_with_dist_type(&t0, &t1, g, DistanceType::Spherical);
    ...
    fn test_erp_difference_between_implementations() {
        ...
        let dist_compat_euclidean = erp_compat_with_dist_type(&t0, &t1, g, DistanceType::Euclidean);
        let dist_standard_euclidean =
            erp_standard_with_dist_type(&t0, &t1, g, DistanceType::Euclidean);
        let dist_compat_spherical = erp_compat_with_dist_type(&t0, &t1, g, DistanceType::Spherical);
        let dist_standard_spherical =
            erp_standard_with_dist_type(&t0, &t1, g, DistanceType::Spherical);
