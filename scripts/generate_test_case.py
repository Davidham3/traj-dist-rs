...
import numpy as np
import pandas as pd
# 使用 Python 实现（traj_dist.pydist）而不是 Cython 实现（traj_dist.cydist）
# 原因：Cython 实现与 Python 实现之间存在微小的精度差异（约 1e-6）
import traj_dist.pydist.sspd as sspd
import traj_dist.pydist.dtw as dtw
import traj_dist.pydist.hausdorff as hausdorff
import traj_dist.pydist.frechet as frechet
import traj_dist.pydist.discret_frechet as discret_frechet
import traj_dist.pydist.lcss as lcss
import traj_dist.pydist.edr as edr
import traj_dist.pydist.erp as erp
import traj_dist.pydist.sowd as sowd


def parse_args():
    parser = argparse.ArgumentParser(description="生成单个算法的测试用例")
...
def main():
    ...
    print(f"Generating test cases for: {args.algorithm}")
    print(f"Parameters: {params}")
    print(f"Sample file: {sample_path}")

    # 获取对应的距离函数（使用 Python 实现）
    if args.algorithm == "sspd":
        dist_func = sspd.e_sspd if args.type_d == "euclidean" else sspd.s_sspd
    elif args.algorithm == "dtw":
        dist_func = dtw.e_dtw if args.type_d == "euclidean" else dtw.s_dtw
    elif args.algorithm == "hausdorff":
        dist_func = hausdorff.e_hausdorff if args.type_d == "euclidean" else hausdorff.s_hausdorff
    elif args.algorithm == "frechet":
        dist_func = frechet.frechet
    elif args.algorithm == "discret_frechet":
        dist_func = discret_frechet.discret_frechet
    elif args.algorithm == "lcss":
        dist_func = lcss.e_lcss if args.type_d == "euclidean" else lcss.s_lcss
    elif args.algorithm == "edr":
        dist_func = edr.e_edr if args.type_d == "euclidean" else edr.s_edr
    elif args.algorithm == "erp":
        dist_func = erp.e_erp if args.type_d == "euclidean" else erp.s_erp
    elif args.algorithm == "sowd_grid":
        dist_func = sowd.sowd_grid
    else:
    ...
    metainfo = {
        ...
        "type_d": args.type_d,
        "implemented_by": "pydist",  # 标识使用 Python 实现
    }
    if args.eps is not None:
    ...
    metainfo["sample_file"] = f"../samples/{sample_filename}"
