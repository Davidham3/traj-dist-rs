...
这个脚本使用现有的benchmark.py脚本的逻辑来生成性能报告，
并将其输出为适合Sphinx文档系统的格式。
"""
...
def generate_performance_report(output_file: Path, num_runs: int = 3):
    """生成性能对比报告到指定文件"""
    print("开始生成性能对比报告...")
    ...
    report_content.append("- **每个测试用例运行次数**: {}次（用于计算平均耗时）\n".format(num_runs))
...
if __name__ == "__main__":
    ...
    generate_performance_report(output_file, num_runs=3)
